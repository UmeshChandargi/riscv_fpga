	...
	ARRAY_FOR_EACH(ipv4->unicast, i) {
		if (!ipv4->unicast[i].ipv4.is_used ||
			...
			continue;
                ...
                /*mcause: 4, Load address misaligned issue. Workaround added to align addr->s_addr */
		uint32_t aligned = 0;
		assert(sizeof(addr->s_addr) == 4);
		memcpy(&aligned, &addr->s_addr, sizeof(addr->s_addr));
		bcast.s_addr = ipv4->unicast[i].ipv4.address.in_addr.s_addr |
			       ~ipv4->unicast[i].netmask.s_addr;
		if (bcast.s_addr == aligned){
			ret = true;
			goto out;
		}
	}
