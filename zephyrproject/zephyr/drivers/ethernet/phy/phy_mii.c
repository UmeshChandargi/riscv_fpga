...
static inline int reg_read(const struct device *dev, uint16_t reg_addr,
			   uint16_t *value)
{
	printf("inside phy_mii.c  reg_read\n");
...
static inline int reg_write(const struct device *dev, uint16_t reg_addr,
			    uint16_t value)
{
	printf("inside phy_mii.c  reg_write\n");
	const struct phy_mii_dev_config *const cfg = dev->config;
...
{
	printf("inside phy_mii.c  is_gigabit_supported\n");
	uint16_t bmsr_reg;
	uint16_t estat_reg;
...
{
	printf("inside phy_mii.c  get_id\n");
	uint16_t value;
...
{
	printf("inside phy_mii.c  update_link_state\n");
	const struct phy_mii_dev_config *const cfg = dev->config;
...
{
	printf("inside phy_mii.c  invoke_link_cb\n");
	struct phy_mii_dev_data *const data = dev->data;
	struct phy_link_state state;
...
{
	printf("inside phy_mii.c  monitor_work_handler\n");
...
static int phy_mii_read(const struct device *dev, uint16_t reg_addr,
			uint32_t *data)
{
	printf("inside phy_mii.c  phy_mii_read\n");
...
static int phy_mii_write(const struct device *dev, uint16_t reg_addr,
			 uint32_t data)
{
	printf("inside phy_mii.c  phy_mii_write\n");
...
{
	printf("inside phy_mii.c  phy_mii_cfg_link\n");
	struct phy_mii_dev_data *const data = dev->data;
	uint16_t anar_reg;
	uint16_t bmcr_reg;
	uint16_t c1kt_reg;
...
static int phy_mii_get_link_state(const struct device *dev,
				  struct phy_link_state *state)
{
	printf("inside phy_mii.c  phy_mii_get_link_state\n");
	struct phy_mii_dev_data *const data = dev->data;
...
static int phy_mii_link_cb_set(const struct device *dev, phy_callback_t cb,
			       void *user_data)
{
	printf("inside phy_mii.c  phy_mii_link_cb_set\n");
	struct phy_mii_dev_data *const data = dev->data;
...
{
	printf("inside phy_mii.c  phy_mii_initialize\n");
	const struct phy_mii_dev_config *const cfg = dev->config;
	struct phy_mii_dev_data *const data = dev->data;
	uint32_t phy_id;
