...
#define CONFIG_SOC_FAMILY_XILINX_ZYNQ7000 1
...
{
#if USR_DBG
	printf("inside eth_xlnx_gem_dev_init\n");
#endif
...
{
#if USR_DBG
	printf("inside eth_xlnx_gem_iface_init\n");
#endif
...
{
#if USR_DBG
	printf("inside eth_xlnx_gem_isr\n");
#endif
	const struct eth_xlnx_gem_dev_cfg *dev_conf = dev->config;
	struct eth_xlnx_gem_dev_data *dev_data = dev->data;
	uint32_t reg_val;
...
{
#if USR_DBG
	printf("inside eth_xlnx_gem_send\n");
#endif
	const struct eth_xlnx_gem_dev_cfg *dev_conf = dev->config;
	struct eth_xlnx_gem_dev_data *dev_data = dev->data;
...
{
#if USR_DBG
	printf("inside eth_xlnx_gem_start_device\n");
#endif
	const struct eth_xlnx_gem_dev_cfg *dev_conf = dev->config;
	struct eth_xlnx_gem_dev_data *dev_data = dev->data;
	uint32_t reg_val;
...
{
#if USR_DBG
	printf("inside eth_xlnx_gem_stop_device\n");
#endif
	const struct eth_xlnx_gem_dev_cfg *dev_conf = dev->config;
	struct eth_xlnx_gem_dev_data *dev_data = dev->data;
	uint32_t reg_val;
...
{
#if USR_DBG
	printf("inside eth_xlnx_gem_get_capabilities\n");
#endif
	const struct eth_xlnx_gem_dev_cfg *dev_conf = dev->config;
...
{
#if USR_DBG
	printf("inside eth_xlnx_gem_stats\n");
#endif
	struct eth_xlnx_gem_dev_data *dev_data = dev->data;
...
{
#if USR_DBG
	printf("inside eth_xlnx_gem_reset_hw\n");
#endif
	const struct eth_xlnx_gem_dev_cfg *dev_conf = dev->config;
...
{
#if USR_DBG
	printf("inside eth_xlnx_gem_configure_clocks\n");
#endif
	/*
...
{
#if USR_DBG
	printf("inside eth_xlnx_gem_set_initial_nwcfg\n");
#endif
	const struct eth_xlnx_gem_dev_cfg *dev_conf = dev->config;
	uint32_t reg_val = 0;
...
{
#if USR_DBG
	printf("inside eth_xlnx_gem_set_nwcfg_link_speed\n");
#endif
	const struct eth_xlnx_gem_dev_cfg *dev_conf = dev->config;
	struct eth_xlnx_gem_dev_data *dev_data = dev->data;
	uint32_t reg_val;
...
{
#if USR_DBG
	printf("inside eth_xlnx_gem_set_mac_address\n");
#endif
	const struct eth_xlnx_gem_dev_cfg *dev_conf = dev->config;
	struct eth_xlnx_gem_dev_data *dev_data = dev->data;
	uint32_t regval_top;
...
{
#if USR_DBG
	printf("inside eth_xlnx_gem_set_initial_dmacr\n");
#endif
	const struct eth_xlnx_gem_dev_cfg *dev_conf = dev->config;
	uint32_t reg_val = 0;
...
{
#if USR_DBG
	printf("inside eth_xlnx_gem_init_phy\n");
#endif
	struct eth_xlnx_gem_dev_data *dev_data = dev->data;
	int detect_rc;

	LOG_DBG("%s attempting to initialize associated PHY", dev->name);
	//printf(">>> %s attempting to initialize associated PHY\n", dev->name);
	...
	//printf(">>> %s detect_rc %d phy_id %08x phy_access_api %p\n", dev->name, detect_rc, dev_data->phy_id, dev_data->phy_access_api);
	...
	if (detect_rc == 0 && dev_data->phy_id != 0x00000000 &&
		...
		printf(">>> %s no compatible PHY detected\n", dev->name);
	}
}
...
{
#if USR_DBG
	printf("inside eth_xlnx_gem_poll_phy\n");
#endif
...
{
#if USR_DBG
	printf("inside eth_xlnx_gem_configure_buffers\n");
#endif
	const struct eth_xlnx_gem_dev_cfg *dev_conf = dev->config;
	struct eth_xlnx_gem_dev_data *dev_data = dev->data;
...
{
#if USR_DBG
	printf("inside eth_xlnx_gem_rx_pending_work\n");
#endif
...
{
#if USR_DBG
	printf("inside eth_xlnx_gem_handle_rx_pending\n");
#endif
	const struct eth_xlnx_gem_dev_cfg *dev_conf = dev->config;
	struct eth_xlnx_gem_dev_data *dev_data = dev->data;
	uint32_t reg_addr;
	...
	while (1) {
		//printf("reading the ring buffer\n");
		...
		if ((reg_val & ETH_XLNX_GEM_RXBD_USED_BIT) == 0) {
			 ...
			 */
			//printf("No new data contained in the current BD. Break out of the loop\n");
			break;
		}
		reg_val = sys_read32(reg_ctrl);
		if ((reg_val & ETH_XLNX_GEM_RXBD_START_OF_FRAME_BIT) == 0) {
			 ...
			 */
			//printf("Although the current BD is marked as 'used', it doesnt contain the SOF bit\n");
		...
		do {
			...
			//printf("iterate until EOF bit is encountered\n");
		...
		do {
			...
			//printf("Copy each BD to the allocated buffer\n");
		...
		if (pkt != NULL) {
			if (net_recv_data(dev_data->iface, pkt) < 0) {
				//printf("Rx packet handover to the IP stack failed\n");
...
{
#if USR_DBG
	printf("inside eth_xlnx_gem_tx_done_work\n");
#endif
	struct eth_xlnx_gem_dev_data *dev_data = CONTAINER_OF(item,
...
{
#if USR_DBG
	printf("inside eth_xlnx_gem_handle_tx_done\n");
#endif
	const struct eth_xlnx_gem_dev_cfg *dev_conf = dev->config;
	struct eth_xlnx_gem_dev_data *dev_data = dev->data;
	uint32_t reg_ctrl;
...
#undef CONFIG_SOC_FAMILY_XILINX_ZYNQ7000
